...
class State(object):
    ...
    def next_states(self, grid):
        ...
        """
        if self.type == EMPTY:
            if not self.possible_succeed(grid):
                back_mirror = self.find_back_mirror()
                if back_mirror:
                    return [back_mirror.get_negitive_mirror_self()]
                else:
                    return [None]
        ...
        elif not self.possible_succeed(grid):
            back_mirror = self.find_back_mirror()
            if back_mirror:
                return [back_mirror.get_negitive_mirror_self()]
            else:
                return [None]
        elif self.type == MINNOR and (self.minor_angle in [SLASH, BACK_SLASH]):
            # a back mirror
            if self.minor_angle == SLASH:
                slash_next_d = slash_next_d_map[self.in_direction]
                slash_n_x, slash_n_y = next_pos_by_direction[slash_next_d](self.x, self.y)
                s_pre_minor = copy(self.pre_minnor_states)
                s_pre_minor.append(self)
                pre_states = copy(self.pre_states)
                pre_states.append(self)
                next_state_af_slash = State(
                    slash_n_x,
                ...
                return [next_state_af_slash]
            elif self.minor_angle == BACK_SLASH:
                back_slash_next_d = back_slash_next_d_map[self.in_direction]
                back_slash_n_x, back_slash_n_y = next_pos_by_direction[back_slash_next_d](self.x, self.y)
                b_slash_pre_minor = copy(self.pre_minnor_states)
                b_slash_pre_minor.append(self)
                pre_states = copy(self.pre_states)
                pre_states.append(self)
                next_state_af_back_slash = State(
                    back_slash_n_x,
                ...
                return [next_state_af_back_slash]

    def get_negitive_mirror_self(self):
        negitive = self.copy()
        if self.minor_angle == SLASH:
            negitive.minor_angle = BACK_SLASH
        if self.minor_angle == BACK_SLASH:
            negitive.minor_angle = SLASH
        # mark as all visited
        ALL_VISITED_MIRRORS.append((negitive.x, negitive.y))
        return negitive

    def find_back_mirror(self):
        n_pre_m = len(self.pre_minnor_states)
        for i in range(n_pre_m):
            # last mirror
            index = n_pre_m - 1 - i
            m = self.pre_minnor_states[index]
            if (m.x, m.y) in ALL_VISITED_MIRRORS:
                continue
            return m
    ...
    def possible_succeed(self, grid):
        return True
        # no need anymore
        remining_m_n = self.get_remining_mirrors_n(grid)
        assert self.type in [EMPTY, MINNOR]
        if self.type == EMPTY:
            out_direction = self.in_direction
        else:
            assert self.minor_angle in [SLASH, BACK_SLASH]
            if self.minor_angle == SLASH:
                out_direction = slash_next_d_map[self.in_direction]
            elif self.minor_angle == BACK_SLASH:
                out_direction = back_slash_next_d_map[self.in_direction]
            # reminiing don't include self
            remining_m_n = remining_m_n - 1
        if out_direction == UP:
            return remining_m_n >=1 and (remining_m_n - 1) % 2 == 0
        if out_direction == RIGHT:
            return remining_m_n >=2 and (remining_m_n - 2) % 2 == 0
        if out_direction == LEFT:
            return remining_m_n >=2 and (remining_m_n - 2) % 2 == 0
        if out_direction == DOWN:
            return remining_m_n >=3 and (remining_m_n - 3) % 2 == 0
...
class Manichie(object):
    ...
    def trans_states(self, states, grid):
        ...
        while True:
            ...
            states = self.get_shortest_next_state(valid_res)
    ...
    def get_shortest_next_state(self, states):
        assert len(states) in [1, 2]
        if len(states) == 2:
            for s in states:
                if s.in_direction in [UP, RIGHT]:
                    return [s]
        elif len(states) == 1:
            return states
...
if __name__ == "__main__":
    ...
    # m04, m05
    result = get_single_solution_from_file('m05.json')
    # print(result)
